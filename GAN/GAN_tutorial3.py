...
def make_conv2d_transpose(inputs, filters, ksize=5, strides=2, padding='SAME', activation=tf.nn.relu):

    layer = tf.layers.conv2d_transpose(inputs, filters=filters, kernel_size=ksize, strides=strides, padding=padding)
    layer = activation(tf.layers.batch_normalization(layer))
    return layer
...
def generator(noise, labels):
    with tf.variable_scope('generator'):
        ...
        # input [None, 100]
        # 7 x 7 x 128
        inputs = tf.layers.dense(noise, 64 * 7 * 7)
        reshape = tf.reshape(inputs, [-1, 7, 7, 64])
        act = tf.nn.relu(tf.layers.batch_normalization(reshape))
        # 14 x 14 x 32
        conv1 = make_conv2d_transpose(act, 32)
        print('cv1 ', conv1.shape)
        # 28 x 28 x 1
        conv2 = make_conv2d_transpose(conv1, 1, activation=tf.nn.sigmoid)
        print('cv2 ', conv2.shape)
        # dropout
        reshape2 = tf.reshape(conv2, [-1, 28*28*1])
